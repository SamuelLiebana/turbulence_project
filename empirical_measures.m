...
load sc_schaefer1000.mat % try with the schaefer 100
C=sc_schaefer;
C=C/max(max(C));
...
% for debug only
Rspatime_av = mean(Rspatime);
mean_global_av = mean(mean_global);
std_global_av = mean(std_global);
edge_matrix_std_av = mean(edge_matrix_std);


% save all the measures in HCP1000_empirical.mat
save HCP1000_empirical_100_subs.mat Rspatime_av mean_global_av std_global_av edge_matrix_std_av;
