...
%        end
    ...
    acftime(sub,:)=autocorr(edgetime(sub,:),100);
    ...
    acftime_su(sub,:)=autocorr(edgetime_su(sub,:),100);
...
save matrix_HCP68_empirical.mat Rspatime_av mean_global_av std_global_av edge_matrix_std_av EdgeSpaTimePredictability_av;
save su_matrix_HCP68_empirical.mat Rspatime_av_su mean_global_av_su std_global_av_su edge_matrix_std_av_su EdgeSpaTimePredictability_av_su;
...
%% autocorr time 
figure(3)
shadedErrorBar(0:101,[1, nanmean(acftime)],[0, nanstd(acftime)],'lineProps', '-r','patchSaturation', 0.2)
%hold on;
